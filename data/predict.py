...
def simulation_fin_de_saison():
    ...
    # Recuperation de tous les matchs termines
    try:
    ...
    # Creation du DataFrame et tri chronologique
    df = pd.DataFrame(data).sort_values(by="date").reset_index(drop=True)
    total_matchs = len(df)
    
    # Separation proportionnelle (70% entrainement / 30% test)
    separation_index = int(total_matchs * 0.70)
    
    df_entrainement = df.iloc[:separation_index]
    df_test = df.iloc[separation_index:]
